...
同时，本Mod对2.5.1的支持已经终止，v0.0.18p28是最后一个明确支持2.5.1的版本，尽管在此之后的版本也许可以在2.5.1正常运行，但原则上不再处理仅在2.5.1出现的不兼容问题。
...
将编程器覆盖板覆盖至GTNH原版输入总线后，该输入总线将能够识别总线内的编程器电路，并消耗该编程电路将该总线的虚拟编程电路设置为与编程器电路相同的电路。

除了原版多方块设备所使用的输入总线外，GT单方块小机器也能使用该覆盖板，同样能更改虚拟电路。

同时，本Mod所提供的二合一输入仓无需该覆盖板，其自带该电路覆盖板功能。
...
除电路板外，其他物品例如催化剂等在配方中不被消耗的物品也能制作为编程电路，如果同时需要虚拟电路和催化剂，可以配合本Mod的多电路槽仓室使用。
...
* 模式3：与模式2几乎相同，但如果该配方没有需要任何不消耗的物品，那么会生成一个将电路板重置为空的编程电路编入样板。
...
其中位置1可存储的物品数量基于电压等级设置，在ME接口向内发送物品时，将不受物品的最大堆叠限制，例如Luv等级的进阶编程缓冲多流体输入仓每格将可堆叠512个物品，EV等级则只能堆叠128个物品，
...
如果你使用的是带缓冲的二合一输入仓，那么位于位置1与位置2的物品或流体会在1tick后被输送到缓冲，此时多方块结构将优先读取0缓冲位置的物品与流体，缓冲内部将有4倍与外部标识的容量，同时各个缓存的虚拟电路与内容物与其他缓存隔离，不会出现配方冲突问题。
...
>设置是否使用可编程电路模式，如果关闭，将不处理进入仓室内的编程电路。
...
与ME接口功能一致，但是能够直接覆盖在机器上作为覆盖板，支持二合一和P2P版本。
...
目前本Mod明确支持GTNH官方非测试版本的所有内容，如果你发现某个仓室无法在某机器上工作，那么该机器本身就不支持总成，除非注入并破坏该机器原本的代码，或者GTNH官方进行修复，不然无法兼容，但是你也还是可以提出相关issue，便于排查问题所在。

同时对于其他社区Mod（如Twist Space Technology Mod）的支持可能不全面与不及时，如果你发现与其他Mod发生兼容性问题请提交issue。
